...
func main() {
	...
	mux.Handle("/auth/", http.StripPrefix("/auth",
		middleware.AuthCorsMiddleware(routes.AuthRoutes(globalAuthConfig))),
	)
	mux.Handle("/tasks/", http.StripPrefix("/tasks",
		middleware.TasksCorsMiddleware(routes.TasksRoutes())),
	)
